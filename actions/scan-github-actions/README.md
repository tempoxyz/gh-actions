...
Both tools run together as a single check (a `Scan GitHub Actions` job in the reusable workflow, or two steps in your own job with the composite action). The **reusable workflow is read-only against Actions and repository data** (`actions: read`, `contents: read`) and never requests `security-events: write`. Callers forward the `STEP_SECURITY_API_KEY` organization secret so Harden Runner can read its StepSecurity policy. SARIF upload to GitHub code scanning is available **only via the composite action** (`advanced-security: true`), which runs in a job you control and where you grant `security-events: write`.
