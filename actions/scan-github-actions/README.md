...
- [ensure-secure-runner](../ensure-secure-runner) — **hardening** (reusable workflow only): every workflow job must start with the [`secure-runner`](../secure-runner) action (Harden Runner plus Aegis). Reusable-workflow calls are allowed; nothing else is exempt.
